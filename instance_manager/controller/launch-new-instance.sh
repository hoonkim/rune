...
while true; do
  scp -i /rune/ssh/id_rsa -oStrictHostKeyChecking=no post_init.sh ubuntu@$IP:.
  if [ $? -ne 0 ]; then
...
ssh -i /rune/ssh/id_rsa -oStrictHostKeyChecking=no ubuntu@$IP ./post_init.sh
UUID=`openstack server show vm$NUM -c id -f value`
SENTINEL=`getent hosts controller | awk '{ print $1 }'`
scp -i /rune/ssh/id_rsa -oStrictHostKeyChecking=no post_script.sh ubuntu@$IP:.
ssh -i /rune/ssh/id_rsa -oStrictHostKeyChecking=no ubuntu@$IP ./post_script.sh $SENTINEL $UUID
